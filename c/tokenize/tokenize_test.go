...
func TestCreateTokenWithPair(t *testing.T) {
	...
	for _, tt := range tests {
		token := createTokenWithPair(tt.in[0], tt.in[1], tt.in[2])
		if diff := cmp.Diff(tt.expect, token); diff != "" {
			t.Errorf("%v", diff)
		}
	}
}
